...
const FILE_TYPES = ['payment_proof', 'other', 'purchase_order', 'gl', 'prescription', 'id', 'dispatch_proof',
  // discord-app: the orders app's Dispatch also photographs the packed parcel
  // and takes a proof of delivery (src/db/migrate.js allows both).
  'packing_proof', 'delivery_proof'];
// discord-app: the photos Dispatch takes, all three reserved to Dispatch.
const DISPATCH_FILE_TYPES = ['dispatch_proof', 'packing_proof', 'delivery_proof'];
...
const DISPATCH_PROOF_STATUSES = [
  ...
  'packed' // discord-app: the orders app's own stop after picking
];
...
function canAttach(user, order, fileType = 'payment_proof') {
  ...
  // discord-app: likewise the packing and delivery photos.
  if (DISPATCH_FILE_TYPES.includes(fileType)) return role === 'dispatch';
  if (role === 'dispatch') return false;
  // discord-app: Finance may add the proof of payment it is verifying against
  // (the orders app's Verify payment step takes one).
  if (role === 'finance') return fileType === 'payment_proof';
...
async function refuseDispatchProof(res, order, fileType) {
  if (!DISPATCH_FILE_TYPES.includes(fileType)) return null;   // discord-app: all three

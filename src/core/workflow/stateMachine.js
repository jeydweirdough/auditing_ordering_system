...
// discord-app: the orders app's own stops (src/workflow/statuses.js). Its steps
// write status themselves, checked against its ACTIONS table; these entries are
// for the Zoho-driven writes that come through orderStatusService (webhooks,
// reconcile), so an order at one of these stops still moves when Zoho reports
// the next thing.
Object.assign(TRANSITIONS, {
  // Team Leader's review, ahead of Management's.
  pending_tl_approval: ['pending_management_approval', 'returned', 'rejected', 'cancelled', 'deleted'],
  // Sent back to its salesperson; resubmitting re-enters review.
  returned: ['pending_tl_approval', 'pending_management_approval', 'cancelled', 'deleted'],
  // Final here: a rejected order is not parked on hold. Admin may still delete it.
  rejected: ['deleted'],
  // Packed, not yet out. Same exits as picking_packing, minus going back to it.
  packed: ['dispatched', 'tracking_shared', 'ready_for_invoice_sent', 'ready_for_dispatch', 'completed', 'on_hold', 'cancelled', 'deleted'],
});
TRANSITIONS.pending_management_approval.push('returned', 'rejected');
TRANSITIONS.picking_packing.push('packed');

...
async function notify({ orderId, recipientIds, message, eventType, orderData = {} }) {
  ...
  // discord-app: where an order is opened, e.g. ORDER_URL_TEMPLATE=https://orders.getmeds.ph/order?id={id}
  const orderLink = process.env.ORDER_URL_TEMPLATE
    ? process.env.ORDER_URL_TEMPLATE.replace('{id}', encodeURIComponent(orderIdentifier))
    : `${appBaseUrl}/orders/${orderIdentifier}`;
  ...
  // discord-app: only to a customer with a real address on file. This used to make one up
  // (patient.<name>@gmail.com), which would mail a stranger once SMTP is switched on.
  if (customerFacingEvents.includes(eventType) && orderData.customer_name && orderData.customer_email) {

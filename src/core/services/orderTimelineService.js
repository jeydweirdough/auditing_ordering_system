...
// discord-app: the orders app's own Mark packed and Mark delivered steps
// (src/workflow/statuses.js) are real, attributed events for these stages.
SPINE.find((s) => s.key === 'packed').types.push('ORDER_PACKED');
SPINE.find((s) => s.key === 'delivered').types.push('DELIVERY_CONFIRMED');

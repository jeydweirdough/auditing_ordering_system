...
function client() {
  ...
  // discord-app: with no Supabase configured and DEV_STORAGE_DIR set (local
  // development and tests only), files go to a folder on this machine through
  // the same signed-URL handshake. src/devStorage.js refuses to load in
  // production.
  if ((!url || !key) && process.env.DEV_STORAGE_DIR) {
    _client = require('../../devStorage').createDevStorageClient();
    return _client;
  }
